...
Comprehensive financial reporting with daily collection reports, fee collection summary, outstanding fees analysis, payment mode breakdown, income/expense reports, custom report builder, and scheduled report generation.

---

## 🎯 SUCCESS CRITERIA

- [ ] Daily collection report with summary functional
- [ ] Fee collection summary by class/category functional
- [ ] Outstanding fees report functional
- [ ] Payment mode analysis functional
- [ ] Income vs expense reports functional
- [ ] Custom report builder functional
- [ ] Scheduled report generation functional
- [ ] Report export (Excel, PDF, CSV) functional
- [ ] Visual charts and graphs functional
- [ ] Comparative analysis reports functional
...
-- Indexes
CREATE INDEX idx_financial_reports_tenant_branch ON financial_reports(tenant_id, branch_id);
...
CREATE INDEX idx_report_cache_tenant_branch ON report_cache(tenant_id, branch_id);
CREATE INDEX idx_report_cache_status ON report_cache(status);
...
-- RLS Policies
CREATE POLICY financial_reports_isolation ON financial_reports
  FOR ALL USING (
...
CREATE POLICY report_cache_isolation ON report_cache
  FOR ALL USING (
    tenant_id = current_setting('app.current_tenant_id')::UUID
    AND branch_id = current_setting('app.current_branch_id')::UUID
  );
...
### API Client (`/lib/api/spec-388-financial-reports-analytics.ts`)
...
export const spec388API = new SPEC388API();
```

### React Component (`/components/02-accountant-portal/FinancialReportsAnalytics.tsx`)
...
### Unit Tests (`/tests/unit/spec-388-financial-reports-analytics.test.ts`)

```typescript
import { describe, it, expect, beforeEach, vi } from 'vitest';
import { spec388API } from '@/lib/api/spec-388-financial-reports-analytics';

describe('SPEC-388: Financial Reports & Analytics API', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('CRUD Operations', () => {
    it('should fetch all records', async () => {
      const result = await spec388API.getAll();
    ...
    it('should create new record', async () => {
      ...
      const created = await spec388API.create(newItem);
      expect(created).toHaveProperty('id');
    });

    it('should update existing record', async () => {
      const updated = await spec388API.update('test-id', {
        name: 'Updated Name'
      });
    ...
    it('should delete record', async () => {
      await expect(spec388API.delete('test-id')).resolves.not.toThrow();
    });
  });
});
```
...
```typescript
import { FinancialReportsAnalytics } from '@/components/02-accountant-portal/FinancialReportsAnalytics';
