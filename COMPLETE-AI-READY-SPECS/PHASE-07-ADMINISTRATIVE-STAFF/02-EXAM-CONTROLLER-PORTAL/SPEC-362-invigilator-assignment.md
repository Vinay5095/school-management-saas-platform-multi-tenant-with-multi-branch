...
Automated invigilator duty assignment system with availability tracking, workload balancing, and notifications.
...
```sql
-- Invigilator Assignment Tables
CREATE TABLE IF NOT EXISTS invigilator_assignment_records (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL REFERENCES tenants(id),
  branch_id UUID NOT NULL REFERENCES branches(id),
  
  -- Core fields specific to Invigilator Assignment
...
CREATE INDEX ON invigilator_assignment_records(tenant_id, branch_id);
CREATE INDEX ON invigilator_assignment_records(status);
CREATE INDEX ON invigilator_assignment_records(created_at DESC);

-- Enable RLS
ALTER TABLE invigilator_assignment_records ENABLE ROW LEVEL SECURITY;

-- RLS Policy
CREATE POLICY invigilator_assignment_records_isolation ON invigilator_assignment_records
  FOR ALL USING (
...
### API Client (`/lib/api/invigilator-assignment.ts`)

```typescript
import { createClient } from '@/lib/supabase/client';

export class InvigilatorAssignmentAPI {
  ...
  async getRecords(params?: {
    ...
    let query = this.supabase
      .from('invigilator_assignment_records')
      .select('*');
...
export const invigilatorAssignmentAPI = new InvigilatorAssignmentAPI();
```

### React Component (`/components/02-exam-controller-portal/InvigilatorAssignment.tsx`)
...
### Unit Tests (`/tests/unit/invigilator-assignment.test.ts`)

```typescript
import { describe, it, expect, beforeEach, vi } from 'vitest';

describe('Invigilator Assignment', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
