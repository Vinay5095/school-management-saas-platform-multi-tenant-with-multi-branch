...
Secure question paper management system with encryption, version control, and controlled distribution.
...
CREATE TABLE IF NOT EXISTS question_paper_management_records (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL REFERENCES tenants(id),
  branch_id UUID NOT NULL REFERENCES branches(id),
  
  -- Core fields specific to Question Paper Management
...
CREATE INDEX ON question_paper_management_records(tenant_id, branch_id);
CREATE INDEX ON question_paper_management_records(status);
CREATE INDEX ON question_paper_management_records(created_at DESC);

-- Enable RLS
ALTER TABLE question_paper_management_records ENABLE ROW LEVEL SECURITY;

-- RLS Policy
CREATE POLICY question_paper_management_records_isolation ON question_paper_management_records
  FOR ALL USING (
...
### API Client (`/lib/api/question-paper-management.ts`)

```typescript
import { createClient } from '@/lib/supabase/client';

export class QuestionPaperManagementAPI {
  ...
  async getRecords(params?: {
    ...
    let query = this.supabase
      .from('question_paper_management_records')
      .select('*');
...
export const questionPaperManagementAPI = new QuestionPaperManagementAPI();
```

### React Component (`/components/02-exam-controller-portal/QuestionPaperManagement.tsx`)
...
### Unit Tests (`/tests/unit/question-paper-management.test.ts`)

```typescript
import { describe, it, expect, beforeEach, vi } from 'vitest';

describe('Question Paper Management', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
export default function Page() {
  return <QuestionPaperManagement />;
}
```
