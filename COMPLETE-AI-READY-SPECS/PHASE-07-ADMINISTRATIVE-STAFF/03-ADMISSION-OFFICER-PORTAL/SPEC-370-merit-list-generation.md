...
Automated merit list generation based on configurable criteria with rank calculation and tie-breaking rules.
...
```sql
-- Merit List Generation Tables
CREATE TABLE IF NOT EXISTS merit_list_generation_records (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL REFERENCES tenants(id),
  branch_id UUID NOT NULL REFERENCES branches(id),
  
  -- Core fields specific to Merit List Generation
...
CREATE INDEX ON merit_list_generation_records(tenant_id, branch_id);
CREATE INDEX ON merit_list_generation_records(status);
CREATE INDEX ON merit_list_generation_records(created_at DESC);

-- Enable RLS
ALTER TABLE merit_list_generation_records ENABLE ROW LEVEL SECURITY;

-- RLS Policy
CREATE POLICY merit_list_generation_records_isolation ON merit_list_generation_records
  FOR ALL USING (
...
### API Client (`/lib/api/merit-list-generation.ts`)

```typescript
import { createClient } from '@/lib/supabase/client';

export class MeritListGenerationAPI {
  ...
  async getRecords(params?: {
    ...
    let query = this.supabase
      .from('merit_list_generation_records')
      .select('*');
...
export const meritListGenerationAPI = new MeritListGenerationAPI();
```

### React Component (`/components/03-admission-officer-portal/MeritListGeneration.tsx`)
...
export function MeritListGeneration() {
...
### Unit Tests (`/tests/unit/merit-list-generation.test.ts`)

```typescript
import { describe, it, expect, beforeEach, vi } from 'vitest';

describe('Merit List Generation', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
```typescript
import { MeritListGeneration } from '@/components/MeritListGeneration';

export default function Page() {
  return <MeritListGeneration />;
}
```
