...
Complete route planning and management system with GPS tracking, stop management, and route optimization.
...
```sql
-- Route Management System Tables
CREATE TABLE IF NOT EXISTS route_management_system_records (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL REFERENCES tenants(id),
  branch_id UUID NOT NULL REFERENCES branches(id),
  
  -- Core fields specific to Route Management System
...
CREATE INDEX ON route_management_system_records(tenant_id, branch_id);
CREATE INDEX ON route_management_system_records(status);
CREATE INDEX ON route_management_system_records(created_at DESC);

-- Enable RLS
ALTER TABLE route_management_system_records ENABLE ROW LEVEL SECURITY;

-- RLS Policy
CREATE POLICY route_management_system_records_isolation ON route_management_system_records
  FOR ALL USING (
...
### API Client (`/lib/api/route-management-system.ts`)

```typescript
import { createClient } from '@/lib/supabase/client';

export class RouteManagementSystemAPI {
  ...
  async getRecords(params?: {
    ...
    let query = this.supabase
      .from('route_management_system_records')
      .select('*');
...
export const routeManagementSystemAPI = new RouteManagementSystemAPI();
```

### React Component (`/components/04-transport-coordinator-portal/RouteManagementSystem.tsx`)
...
export function RouteManagementSystem() {
...
### Unit Tests (`/tests/unit/route-management-system.test.ts`)

```typescript
import { describe, it, expect, beforeEach, vi } from 'vitest';

describe('Route Management System', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
```typescript
import { RouteManagementSystem } from '@/components/RouteManagementSystem';

export default function Page() {
  return <RouteManagementSystem />;
}
```
